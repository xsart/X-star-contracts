...
import {ISwapERC20} from "./swap-interfaces/ISwapERC20.sol";
import {IERC20} from "./swap-interfaces/IERC20.sol";
import {ISwapPairCallee} from "./swap-interfaces/ISwapPairCallee.sol";
import {SafeMath} from "./swap-libs/SafeMath.sol";
import {UQ112x112} from "./swap-libs/UQ112x112.sol";
...
contract SwapPair is ISwapPair, SwapERC20 {
    ...
    function burn(
        ...
        require(
            amount0 > 0 && amount1 > 0,
            "SwapPair: INSUFFICIENT_LIQUIDITY_BURNED"
        );
        ...
        _update(balance0, balance1, _reserve0, _reserve1);
        if (feeOn) kLast = uint256(reserve0).mul(reserve1); // reserve0 and reserve1 are up-to-date
        emit Remove(msg.sender, amount0, amount1, to);
    }
    ...
    function swap(
        ...
        require(
            ...
            "SwapPair: INSUFFICIENT_OUTPUT_AMOUNT"
        );
        (uint112 _reserve0, uint112 _reserve1, ) = getReserves(); // gas savings
        require(
            amount0Out < _reserve0 && amount1Out < _reserve1,
            "SwapPair: INSUFFICIENT_LIQUIDITY"
        );
        ...
        uint256 balance1;
        {
            ...
            if (data.length > 0)
                ISwapPairCallee(to).SwapPairCall(
                    msg.sender,
                    amount0Out,
                    amount1Out,
                    data
        ...
        require(
            ...
            "SwapPair: INSUFFICIENT_INPUT_AMOUNT"
        );
        {
    ...
    function burnToken(address token, uint amount) external lock {
        require(msg.sender == ISwapFactory(factory).router(), "invalid caller");
